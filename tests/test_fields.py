...
Tests for `drf_compound_fields.fields` module.
...
class TestPartialDictField(unittest.TestCase):
    """
    Tests for the PartialDictField
    """

    def test_from_native_with_included_keys(self):
        """
        When a PartialDictField has an included_keys, from_native should return
        a dict of elmenents resulting from the application of the value-field's
        from_native method to values of the input data dict that are includeded
        by included_keys.
        """
        field = PartialDictField(included_keys=['a'], value_field=DateField())
        data = {"a": "2000-01-01", "b": "2000-01-02"}
        obj = field.from_native(data)
        self.assertEqual({"a": date(2000, 1, 1)}, obj)

    def test_to_native_with_included_keys(self):
        """
        When a PartialDictField has an included_keys, to_native should return
        a dict of elmenents resulting from the application of the value-field's
        to_native method to values of the input object dict that are included
        by included_keys.
        """
        field = PartialDictField(included_keys=['a'],
                                 value_field=DateField(format=ISO_8601))
        obj = {"a": date(2000, 1, 1), "b": date(2000, 1, 2)}
        data = field.to_native(obj)
        self.assertEqual({"a": "2000-01-01"}, data)

    def test_from_native_with_nonexisting_included_keys(self):
        """
        When a PartialDictField has an included_keys that includes nonexisting
        keys, from_native will ignore them.
        """
        field = PartialDictField(included_keys=['a', 'c'],
                                 value_field=DateField())
        data = {"a": "2000-01-01", "b": "2000-01-02"}
        obj = field.from_native(data)
        self.assertEqual({"a": date(2000, 1, 1)}, obj)

    def test_to_native_with_nonexisting_included_keys(self):
        """
        When a PartialDictField has an included_keys that includes nonexisting
        keys, to_native will ignore them.
        """
        field = PartialDictField(included_keys=['a', 'c'],
                                 value_field=DateField(format=ISO_8601))
        obj = {"a": date(2000, 1, 1), "b": date(2000, 1, 2)}
